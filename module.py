...
출력은 반드시 JSON 객체여야 합니다. 다음 스키마 예시를 따릅니다.
{
    "scenes":[
        {
            "scene_id": int,
...
반드시 4개 장면을 제출합니다.
prompt_en은 반드시 영어로 작성하고 wide shot/medium shot/close-up, eye-level/low/high angle,
soft/rim/backlit lighting 포함합니다.
scene_ids 는 1부터 시작하는 점수입니다.
"""
...
def extract_scenes(diary_text:str) -> list[dict]:
    ...
    if not api_key:
        raise RuntimeError("OPEN_API_KEY 가 .env에 있는지 확인합니다")
    Client=OpenAI()
...
def validate_scenes(scenes:list[dict])->list[str]:
    """scene 리스트가 4장인 필수 7개 필드(scene_id, scene_kr, shot, angle, lighting, lens, prompt_en) 충족하는지 검증합니다"""
    errors: list[str] = []
    if len(scenes) != 4:
